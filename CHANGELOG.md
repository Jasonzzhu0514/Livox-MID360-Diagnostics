...
## [2.1.0] - 2026-07-21
### 新增
- GUI 新增 PointCloud Preview，使用 Livox-SDK2 获取实时点云，并通过 Three.js 提供交互式预览。
- GUI Diagnostics 新增有线网卡选择、链路检查和临时 `192.168.1.5/24` 配置/释放操作，未预先配置网卡时也能完成雷达发现。
- 新增 About 对话框，显示工具用途、版本、作者信息和项目开源地址。
### 优化
- 点云预览改为仅传输和渲染最新完整帧，使用动态 GPU 缓冲区，不再累积历史帧，改善渲染速度和内存占用。
- 统一 monitor、preview 和 config tool 的 SDK 网卡候选策略；GUI 管理临时网络配置，后端命令禁用重复 auto-bind。
- 收紧 GUI 布局、控制项和日志/状态展示；诊断与点云预览分离运行结果，日志页只显示最近一次执行输出。
### 修复
- 修复点云预览反复启动、切换页面或收到旧运行事件时可能残留旧数据或状态错乱的问题。

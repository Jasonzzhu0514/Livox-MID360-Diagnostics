...
- `livox_mid360_diagnostics autoconfig` / `livox-mid360-diagnostics autoconfig` 运行时会执行网络发现；交互选择配置文件或显式使用 `--apply` 时才会更新文件。
- `livox-mid360-diagnostics udp-monitor` 只绑定本机 UDP 端口并观察收到的数据包，不会向雷达发送命令。
- `livox_mid360_diagnostics monitor` 会初始化 Livox-SDK2，默认通过 SDK discovery 发现雷达并显示回调状态；如果使用相应参数，还可以请求 normal mode、点云发送或 IMU 发送。
- `livox_mid360_diagnostics dump` 会初始化 Livox-SDK2 并把回调数据解析成 CSV；CSV 只建议短时采样，默认不主动发送控制请求，只有带相应参数时才会请求 normal mode、点云发送或 IMU 发送。
